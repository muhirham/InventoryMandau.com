...
{
    /** Halaman Stock Gudang (read-only) */
    public function index(Request $r)
    {
        $me = auth()->user();
        return view('wh.stockGudang', compact('me'));
    }
    ...
    public function datatable(Request $r)
    {
        try {
            ...
            $me          = auth()->user();
            $warehouseId = $me->role === 'warehouse'
                ? ($me->warehouse_id ?? null)
                : ($r->integer('warehouse_id') ?: null);
            ...
            if ($hasPackages)   $base->leftJoin('packages as pk', 'pk.id', '=', 'products.package_id');
            if ($hasCategories) $base->leftJoin('categories as c', 'c.id', '=', 'products.category_id');
            if ($hasSuppliers)  $base->leftJoin('suppliers as s', 's.id', '=', 'products.supplier_id');

            $base->select([
                DB::raw('products.id as product_id'),
                DB::raw('products.product_code'),
                DB::raw('products.name as product_name'),
                $pkgNameCol ? DB::raw("COALESCE(pk.$pkgNameCol, '-') as package_name") : DB::raw("'-' as package_name"),
                $catNameCol ? DB::raw("COALESCE(c.$catNameCol, '-') as category_name") : DB::raw("'-' as category_name"),
                $hasSuppliers ? DB::raw("COALESCE(s.name, '-') as supplier_name") : DB::raw("'-' as supplier_name"),
            ...
            if ($search !== '') {
                ...
                $base->where(function ($q) use ($like, $pkgNameCol, $catNameCol, $hasSuppliers) {
                    ...
                    if ($pkgNameCol)   $q->orWhere("pk.$pkgNameCol", 'like', $like);
                    if ($catNameCol)   $q->orWhere("c.$catNameCol", 'like', $like);
                    if ($hasSuppliers) $q->orWhere('s.name', 'like', $like);
                });
            }

            // Hitung total & filtered
            $totalBase = DB::table('products')
                ->joinSub($stockSub, 'st', 'st.product_id', '=', 'products.id');
            $recordsTotal = $totalBase->count('products.id');
            ...
            // Paging + render (kasih alias 'name' biar kompatibel lama)
            ...
            Log::error('StockLevel.datatable error: '.$e->getMessage(), ['trace' => $e->getTraceAsString()]);
            return response()->json([
                'draw' => (int) $r->input('draw', 1),
                'recordsTotal' => 0,
                'recordsFiltered' => 0,
