    ...
    :root{
        ...
        --glass-br: 16px; --glass-blur: 12px;
        --shadow: 0 10px 30px rgba(15,23,42,.12);
        --shadow-hover: 0 16px 42px rgba(15,23,42,.18);
        --muted:#6b7280;
    }
    ...
    .wh-inner{ position:relative;width:100%;height:240px;transform-style:preserve-3d;transition:transform .65s cubic-bezier(.22,.61,.36,1),box-shadow .25s ease,translate .25s ease; }
    .wh-inner:hover{ translate:0 -4px; box-shadow:var(--shadow-hover); }
    .wh-inner.flipped{ transform:rotateY(180deg); }

    .wh-face{ position:absolute; inset:0; backface-visibility:hidden; border-radius:var(--glass-br);
    ...
    .btn-3d{ border-radius:50%; width:56px; height:56px;
    ...
    function filtered(){ if(!keyword) return warehouses; const q=keyword.toLowerCase(); return warehouses.filter(w=>[w.warehouse_code,w.warehouse_name,w.address,w.note].join(' ').toLowerCase().includes(q)); }
    ...
    function render(){
        ...
        <div class="col-sm-6 col-lg-4">
            ...
            <div class="wh-inner d-flex align-items-center justify-content-center">
                <button class="btn-3d" title="Add warehouse"><i class="bx bx-plus fs-3"></i></button>
            </div>
            </div>
        </div>
        `);
        ...
        pager.empty();
        const tp = totalPages();
        pager.append(`<li class="page-item ${currentPage===1?'disabled':''}"><a class="page-link" data-goto="${currentPage-1}">«</a></li>`);
        for(let p=1;p<=tp;p++){
        pager.append(`<li class="page-item ${p===currentPage?'active':''}"><a class="page-link" data-goto="${p}">${p}</a></li>`);
        }
        pager.append(`<li class="page-item ${currentPage===tp?'disabled':''}"><a class="page-link" data-goto="${currentPage+1}">»</a></li>`);
    }
    ...
    // Search
    $('#searchBox').on('input', function(){ keyword=this.value||''; currentPage=1; render(); });

    // Pager click
    pager.on('click','.page-link', function(){ const p=parseInt(this.dataset.goto,10); if(!isNaN(p)) goto(p); });

    // flip edit
    grid.on('click','.js-edit', function(e){
        e.stopPropagation();
        const inner=$(this).closest('.wh-inner'); inner.addClass('flipped');
    ...
    grid.on('click','.js-del', async function(e){
        ...
        // adjust page jika kosong
        const before = totalPages();
        currentPage = clamp(currentPage);
        const after = totalPages();
        if(currentPage>after) currentPage = after;
    ...
    grid.on('submit','.edit-form', async function(e){
        ...
        if(!payload.warehouse_code || !payload.warehouse_name) return warnBx('Validation','Code & Name are required.');
        ...
        if(res.status===422){
            const j=await res.json(); return warnBx('Validation', Object.values(j.errors||{}).flat().join('<br>'));
        }
        if(!res.ok) throw new Error('Failed: '+res.status);
    ...
    grid.on('click','#cardAdd', function(){
        Alert.fire({
        title:'Add Warehouse',
        html:`<input id="sw_code" class="form-control mb-2" placeholder="Code">
        ...
        preConfirm:()=>{
            const code=$('#sw_code').val().trim(), name=$('#sw_name').val().trim();
            if(!code || !name){ Alert.showValidationMessage('Code & Name required'); return false; }
            return { warehouse_code:code, warehouse_name:name, address:$('#sw_addr').val().trim(), note:$('#sw_note').val().trim() };
        }
        }).then(async r=>{
